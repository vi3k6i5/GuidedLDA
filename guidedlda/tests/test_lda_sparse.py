...
class TestLDASparse(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.dtm = scipy.sparse.csr_matrix(guidedlda.utils.ldac2dtm(open(reuters_ldac_fn), offset=0)).astype(np.int64)
        cls.n_iter = n_iter = 1
        cls.n_topics = n_topics = 10
        cls.random_seed = random_seed = 1
        cls.model = guidedlda.GuidedLDA(n_topics=n_topics, n_iter=n_iter, random_state=random_seed)
